...
test("pinned shell identifies the Application and keeps Runs state in basename URLs", async ({ page }) => {
  ...
  await expect(page.getByRole("option", { name: "Executing 5" })).toBeVisible();
  await page.getByRole("option", { name: "Executing 5" }).click();
  ...
  await page.getByRole("heading", { name: "Runs" }).click();
  await page.keyboard.press("f");
  await page.getByRole("option", { name: "Trace 2" }).click();
  ...
  await page.getByRole("option", { name: "1 day 7" }).click();
