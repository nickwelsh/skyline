...
const axeImpacts = new Set([null, "minor", "moderate", "serious", "critical"]);
...
export function validateNw223Ledger(ledger: Nw223EvidenceLedger) {
  assertObjectKeys("ledger", ledger, ["axe", "interactions", "measurements"]);
  ...
  for (const capture of expectedNw223AxeCaptureIds) {
    ...
    assertObjectKeys("interaction pair", interaction, ["skyline", "trigger"]);
    assertObjectKeys("Axe pair", ledger.axe[capture], ["skyline", "trigger"]);
...
function assertMeasurement(capture: string, measurement: PresenterExtensionMeasurement) {
  ...
  if (typeof measurement.anchorAccessibleName !== "string" || measurement.anchorAccessibleName.trim().length === 0) throw new Error(`Invalid NW-223 measurement accessible name for ${capture}.`);
}
...
function assertAxePartition(capture: string, application: "trigger" | "skyline", partition: PartitionedAxeLedger) {
  ...
  for (const scope of ["outside", "inside"] as const) {
    ...
    for (const rule of partition[scope]) {
      ...
      if (typeof rule.id !== "string" || rule.id.length === 0 || rule.id.includes("*")) throw new Error(`Invalid NW-223 Axe rule for ${application} ${capture}.`);
      if (!axeImpacts.has(rule.impact ?? null)) throw new Error(`Invalid NW-223 Axe impact for ${application} ${capture}.`);
      ...
      if (!Array.isArray(rule.targets) || rule.targets.length === 0 || rule.targets.some((target) => typeof target !== "string") || new Set(rule.targets).size !== rule.targets.length) {
      ...
      for (const target of rule.targets) {
        ...
        if (!Array.isArray(path) || path.length === 0 || path.some((segment) => typeof segment !== "string" || segment.length === 0 || segment.includes("*") || !validSelector(segment))) {
          throw new Error(`Invalid NW-223 Axe target for ${application} ${capture}.`);
        }
        const canonical = JSON.stringify(path.map((segment) => segment.trim()));
        if (paths.has(canonical)) throw new Error(`Duplicate NW-223 Axe target path for ${application} ${capture}.`);
        paths.add(canonical);
      }
    }
  }
