...
export function HelpMenu({ collapsed, capabilities, onOpenShortcuts, labelOpacity }: { collapsed: boolean; capabilities: HelpCapabilities; onOpenShortcuts: () => void; labelOpacity: number }) {
  return <Popover><PopoverTrigger className={cn("flex h-8 items-center gap-1.5 rounded pl-1.75 pr-2 text-text-dimmed hover:bg-background-hover hover:text-text-bright focus-custom", collapsed ? "w-8" : "min-w-0 flex-1")}>
    ...
    {capabilities.shortcuts && <button type="button" data-action="shortcuts" onClick={onOpenShortcuts} className="flex h-8 w-full items-center gap-2 rounded px-2 text-left hover:bg-background-hover focus-custom"><KeyboardIcon className="size-5 text-text-dimmed" /><span className="flex-1">Shortcuts</span><ShortcutKey shortcut={{ modifiers: ["shift"], key: "?" }} variant="medium" /></button>}
